...
- They will not replace R skills — they amplify them
- But only if you know how to verify what they give you
]
...
.footnote[This session assumes no paid subscriptions. Everything runs locally or via free tiers.]
...
**For this course:** We focus on **Ollama** (local) and **Wikipedia API** (structured knowledge) because they require zero subscriptions and work offline.
...
**What to say:**
> "If you want to follow along with local models, install Ollama from ollama.com and run `ollama pull llama3.2` in your terminal. If you cannot install software, we also show API-based alternatives you can call from R with an internet connection."
...
## Principle 1: The Prompt is Part of Your Method
...
## Principle 2: Use `ellmer` — One Interface, Many Models

**What to say:**
> "The `ellmer` package from Posit is the modern way to talk to LLMs from R. It abstracts away provider differences. You can switch from a local Ollama model to an OpenAI API endpoint by changing one line of code. This is crucial for reproducibility — your analysis logic stays the same even if the backend changes."
...
## Principle 3: Structured Output — Force Precision
...
## Principle 4: Grounding with Wikipedia API

**What to say:**
> "LLMs hallucinate facts. A cheap and effective way to ground them is to fetch structured knowledge from Wikipedia via the WikipediR package, then feed that text into the prompt as context. This is a primitive form of Retrieval-Augmented Generation (RAG) — and it works remarkably well for domain concepts your students might ask about."
...
## Live Demo: Wikipedia + LLM Pipeline
...
## Principle 5: Iterative Refinement — The Chat Thread
...
# Likely result: generic, wordy, no code, wrong examples
```
]
...
## Principle 6: LLMs for Code Explanation, Not Just Generation

**What to say:**
> "The most underrated use of LLMs is explaining code you inherited. Paste a gnarly base R snippet into the chat and ask for a tidyverse translation with line-by-line comments. This is safer than generation because the input code is ground truth — the LLM only restructures it."
...
```{r task2-refactored}
# LLM-refactored (expected output)
refactored <- penguins %>%
...
**What to say:**
> "LLMs are confident liars. They invent R packages, hallucinate function arguments, and generate code that runs but produces wrong results. Your job as a data analyst is to be a skeptic. Here are common failure modes with penguin data."
...
# "Remove all missing values" — dangerous!
penguins_clean <- na.omit(penguins)

# "Standardize all numeric variables" — includes year!
...
# "Stepwise selection to find best model"
...
**Key insight:** The LLM produced *syntactically valid* R code that is *methodologically catastrophic*. Syntax checkers will not catch this. Only domain knowledge does.

---

## Principle 8: LLMs + Wikipedia for Domain Exploration

**What to say:**
> "Before you analyze data, you need domain context. Wikipedia is a structured, versioned, and citation-linked knowledge base. Combining WikipediR with LLMs gives you a cheap literature-review assistant. The LLM synthesizes; Wikipedia grounds."
...
**Limitation:** Wikipedia is a starting point, not a primary source for publication.
]
...
## Principle 9: Automation — Batch-Processing with LLMs
...
3. Rewrite ONE chunk correctly with a comment explaining your fix.
4. Write a 3-bullet "Guidelines for AI-Assisted Homework" for the class.

---

## TASK 5: Review Key Points
...
| Filtering by p < 0.05 | Yes | Report full table with effect sizes; pre-register hypotheses |

**Class guideline:**
- **Verify every data transformation** against domain knowledge.
- **Never modify data based on a global statistic** (IQR, mean) without stratifying by known groups.
- **Treat LLM output as a first draft** — peer review it as you would a colleague's code.

---

## Summary: The LLM-Aware Research Cycle

.pull-left[
**The workflow**

1. Ask Wikipedia (structured grounding)
2. Draft prompt (role, audience, constraints)
3. Generate code (structured output preferred)
4. Audit & verify (syntax + methodology)
5. Run in R (you are the executor)
6. Log provenance (prompts are methods)
]
...
- `ellmer` for provider independence
- Wikipedia for grounded generation
- Prompts are part of methods
- LLMs generate drafts, not decisions
- The most dangerous code is code that runs
]
