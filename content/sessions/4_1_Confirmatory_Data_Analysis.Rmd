...
```{r se-lm}
summary(fit_log_step)$coefficients[,2]
```
...
confint(fit_log_step)

#- for odds ratio (OR) from log-odds
# - i.e. OR < 1 predictor is associated with lower odds of the outcome
exp(confint(fit_log_step))
...
---
## Coefficient Plots (parameters)

Create coefficient plot
```{r fig.width= 5, fig.height=4}
library(parameters)

model_parameters(fit_log2) %>% 
  plot()
```

---
## ggplot2-based coefficient plot
...
model_parameters(fit_log2) %>% 
  plot() +
  scale_y_discrete(
    labels = c(  # reversed order
      "Sentiment on AI", "Age", "Professional stage"
    )
  ) +
  theme_minimal() +
...
Does age moderate the relationship between r used and main_branch?

```{r interaction model}

fit_log2_interaction <- glm(r_used ~ main_branch:age_group +
                  ai_sentiment,
                  family = binomial, data = df_model)

model_parameters(fit_log2_interaction)
```
...
Interaction parameters are difficult to interpret. Hence, plotting the regression parameters with `sjPlot`

```{r fig.width= 5, fig.height=4}

fit_log2_interaction %>% 
  plot_model(type = "int") +
  xlab("Age") +
  ylab("'R used") +
