...
subtitle: "Basic Data Structures & Importing"
...
Operations are applied to all elements of a vector automatically.

```{r vectors}
numeric_vector   <- c(1, 2, 3, 4)
character_vector <- c("a", "b", "c", "d")
 
numeric_vector[3] #inspect entry 3 from vectornumeric_vector

length(character_vector) # vector length
```
...
They are a collection of elements of the **same type** (string, numeric OR logical).
...
students <- data.frame(
  ...
  Major = c("Biology", "Math", "History", "Economics", "Physics"),
  Grade = c("A", "B", "A", "C", "B")
)
...
fancy_nested_list <-
  list(
    ...
    data_stuff = list(fancy_matrix, students)
  )
...
```{r changing-names}
names(students) <- c("name", "age", "studies", "performance")
...
- A. Create a vector `fav_music` with the names of your favorite artists.

- B. Create a vector `num_records` with the number of songs you have in your collection for each artist.

- C. Create a vector `num_concert` with the number of times you attented the concerts for each artist.

- D. Combine everything into a **data frame**, name it as `my_music`. Change the labels of the data in the columns `artist`, `records`, and `concerts`.

- E. Extract the variable `num_records` from the data frame `my_music`.
...
```{r}
fav_music <- c("Muse", "Mozart", "Britney Spears", "Nirvana")
num_concerts <- c(2, 0, 1, 0)
num_records <- c(4, 2, 1, 8)
...
- Getting the most common data formats into `R`
  - e.g., CSV, *.txt*, or *Excel* spreadsheets
...
path.donuts <- file.path(path, "donut.txt")
path.donuts

# inspect path name
donuts <- read.table(path.donuts, header = FALSE,
col.names = c("type", "calories", "sodium"))

str(donuts)# inspect data imported
```


```{rdonut2, eval=FALSE}
donuts2 <- read.table(path.donuts, header = FALSE,
col.names = c("type", "calories", "sodium"),
